...
`pyproject.toml` has runtime `dependencies = ["pytest>=9"]` (needed by `cambium bench`/`cambium module-test`) and a `dspy` extra; `requires-python` is `>=3.14` with no packaging upper bound.
