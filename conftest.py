...
pyproject.toml's ``pythonpath = ["src"]`` only adds the source tree to the
pytest parent's sys.path. Scenario tests spawn subprocesses (``-m
cambium.worker``, ``-m cambium.bench``, ``-m cambium.cli``) that inherit
os.environ, so export the source tree via PYTHONPATH as well; both edits
happen here, before collection.
"""
