...
| `context_checkpoint` | `worker.py`; forwarded by `supervisor.py` | Records an immutable epoch checkpoint and its cache descriptor (compatibility identity, not provider cache-hit evidence). |
...
| `merge_failed` | `supervisor.py` | Records merge/publication failure; conflicts carry a structured `merge_conflict` status. A private-integration observer diagnostic may carry `internal=true,recoverable=true` after the integration head is durable; that diagnostic does not reject the task result. |
...
| `response_chunk` | `supervisor.py` (worker `response_chunk` message in `worker.py`) | Critical durable finish-response chunk. Chunks carry exact task/generation/request identity, zero-based contiguous indexes, bounded UTF-8 text (32 KiB per frame; 4 MiB raw ingress and 129 chunks per generation; 16 MiB post-redaction durable output and 513 chunks per generation; 64 MiB raw and 256 MiB durable cumulative session guards), and one final marker. These byte and chunk bounds are transport, redaction-expansion, and backpressure resource guards; they are not a product answer-size target or ceiling. The supervisor buffers the bounded raw sequence, applies session redaction to the assembled response, then rechunks it before persistence; raw text is never persisted or sent to observers. Exact duplicates are ignored; stale, conflicting, gapped, malformed, or post-terminal chunks cannot extend the response. The bounded `result` metadata (`response_chunk_count`, `response_bytes`) lets replay rebuild the valid durable prefix and mark a complete response only after a successful terminal result that was not later rejected by supervisor lifecycle checks; full response text is not copied into the result envelope. |
| `result` | `supervisor.py` (worker envelope in `worker.py`) | Records an accepted terminal worker verdict only when the envelope's `request_id`, `task_id`, and `generation` exactly match the dispatched run. Rejected terminal envelopes still emit a bounded `response_valid=false` marker for replay fencing; accepted records carry bounded `terminal_action` (`type`, `objective_met`, and summary presence) and redacted provider metadata. |
