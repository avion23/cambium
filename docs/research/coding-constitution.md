...
- **One uncited premise corrected:** the brief's `worker-coldstart.md` does not exist in this worktree; the verified cold-start evidence is `review-implementation.md` §M2 (1–3 s `import dspy`) and `dspy-python-314.md` (§Recommendation: 57 packages, lazy-import policy).
...
- `agents.md` §7 is "Coding norms specific to Cambium" (flat bullets, no `###` subheadings today); §8 follows. The patch inserts a `###` subsection at the end of §7 — structurally minimal, no renumbering needed.
- The patch keeps every existing §7 bullet untouched; it restates (with pointers) the bullets it overlaps and adds only the genuinely new ones: measurement-first, no-shared-mutable-state framing, loop-affine state, interface minimalism, delete-over-add.
...
| 6 | agents.md §7 norms (flat control flow, enums, no globals, no frameworks) | all four bullets present verbatim | `agents.md:107-116` |
...
Ready to apply verbatim. Insert after the last §7 bullet ("Every disk write off the event loop."),
before §8 "Where to look for what". ~50 lines (the fenced block below).

```
## (patch) INSERT into agents.md — after §7's last bullet, before §8
...
- **Business logic = pure functions on flat structs; state and I/O at the edges.** `Module.decide()`
  is the seam (`docs/module-template/architecture.md` §5.1). Rule engines stay pure
  and stateless like `should_decompose` (`src/cambium/modules/example/decide.py`). See (d).
...
- **Small, JSON-schema-shaped interfaces; modules deletable without breaking siblings.** A
  module's contract is its frozen-dataclass inputs/outputs (`docs/module-template/
  architecture.md` §3). Pinned siblings (`docs/architecture.md` §17.2) keep each module
  independently optimizable and removable; §9 "done" is the deletion checklist. See (g).
...
subsection adds 12 bullets; no §8 renumbering required.
