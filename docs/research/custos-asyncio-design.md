...
There is **no test-strategy document in `main`** at this commit (`docs/` contains only `research/`, `reviews/`, `system-design.md`). The harness strategy lives in the un-merged arch worktree: `docs/module-template/architecture.md` §9 (unit tests, eval harness, canary suite, integration smoke test, sibling pinning) and the scaffold's `tests/scenarios/test_example_module.py` precedent (real fixtures, no mocks). The scenarios below slot into that strategy's §9.4 integration tier and extend the smoke test.
...
- `/home/ubuntu/cambium/docs/reviews/review-distributed-systems.md` — DS-C1 (§1, §2), DS-M2 (§3), DS-M3 (§2.4).
- `/home/ubuntu/cambium/docs/system-design.md` §M4 (lines 397–643) — the reviewed `Supervisor` (this doc replaces its I/O and liveness model).
- `/tmp/opencode/cambium-arch/docs/architecture.md` (v2.0.0, arch worktree, pending merge into main) — §5.3 liveness model, §6 event-log writer (§6.2) and durability contract (§6.5), §7.1 state machine, §7.2 spawn, §7.3 fencing, §7.4 restart, §7.5 worktree recovery, §7.7 shutdown, §7.8 Unio publish, §13 logging, §14 Python stance (`asyncio.to_thread`).
- `/tmp/opencode/cambium-arch/docs/module-template/architecture.md` §9 — the test-strategy template this doc's §6 extends (not yet in `main`).
