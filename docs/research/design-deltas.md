...
- The v2 design already carried the claim that sandbox ≠ primary isolation: `architecture.md` §19 lists worktree recovery, fencing, and process-group kill as the liveness/split-brain controls, and the threat model's own summary ranks `run_shell` containment "on Linux (bwrap + network off)" as the only sandbox-dependent piece (`docs/research/threat-model.md` §3.3, §5 R3). D7 re-bases that piece on allowlists + gates.
