...
- `docs/system-design.md` (v0.1, superseded), `docs/reviews/*` (v0.1 adversarial reviews), and `docs/research/*` — **as merged in `main`** (present in this worktree).
...
| `docs/research/threat-model.md` | `wt-threat` @ `b863084` | Threat model (R1..R10) — cited by D7 |
| `docs/research/sandbox-options.md` | `wt-sandbox` @ `242a509` | Septum sandbox options on this host — retained as evidence by D7 |
| `docs/research/worker-coldstart.md` | `wt-coldstart` @ `108c83d` | fork-per-task vs persistent-pool benchmark — cited by D3 |

These are the same stable-path references the v2 architecture itself uses for unmerged research (`architecture.md` §20: "references here are by stable path so coherence is auditable when the merge lands").

### 0.4 Summary

| Delta | Source | Status | Amends | One line |
|---|---|---|---|---|
| D1 | USER DIRECTIVE | adopt | §2, §4 (M2), §8, §8.1, §9.2, §9.3, §18.1 (LLM-C1), §18.4 (F2) | Delete the Diffundo local cache; Diffundo becomes a stateless router; provider-side caching is content-addressed and never stale. |
...
| D3 | PRIME AGENT | adopt | §5.1, §5.2, §6.4, §7.2, §14 | Persistent named worker sessions: `spawn`→`admission` (async handle), `steer` by `session_id`, child→parent result messages; sessions checkpointed and reloadable. |
...
| D7 | USER DIRECTIVE | adopt | §2, §4 (M8), §7.2, §11, §12.1, §19, §18.3 (IMPL-M4) | No sandboxing in the harness: Septum removed from v2 scope; containment = worktree isolation + permission allowlists + approval gates; least-privilege worker env (R4). |
...
**Status:** **adopt**
**Amends:** `docs/architecture.md` §2 (layering diagram, Diffundo line), §4 (module catalog, M2 row), §8 (transparency table, Diffundo row), §8.1 (deleted/replaced), §9.2 (cascade `call` step 1), §9.3 (worker-side `CambiumLM` cache flags), §18.1 (LLM-C1 row), §18.4 (F2 row reference), §18.2 (LLM-M5 row); `docs/system-design.md` §M2 (superseded, for history).
...
**Status:** **adopt**
**Amends:** `docs/architecture.md` §3.4 (`Result` envelope), §3.6/§6.3 (Event schema), §4 (Architectus — decomposition produces the tree), §7.1 (per-task state machine gains a tree-level completion rule), §16 (host navigation of the session tree); `docs/module-template/example-spec.md` §3 (decomposition output feeds the tree).
...
2. **Each node owns its conversation/session log.** The node's full conversation (spec, steering turns, tool events, checkpoints) lives under its session store (`${session_dir}/cambium/sessions/<node_id>/`, aligning with §16.2 layout), append-only. This is the durable memory of the sub-LLM session (Prime Agent's session-tree precedent: `docs/research/prime-agent.md` §2 "Sessions: append-only JSONL ... with a tree (`id`/`parentId`); in-place branching, `/fork`, `/clone`").
...
4. **Event log records `parent_task_id`.** The `events` table (`architecture.md` §6.3) gains a `parent_task_id` column; `task_assigned` and per-task events carry it. This lets the tree be reconstructed from the log and lets the host navigate the session tree (§16).
5. **Invariants (normative):**
   ...
   - I2.4 **Context composition.** Node context = its own session log (bounded) + parent summary + subtree result envelopes. A node never reads a sibling's raw session; siblings communicate only through the parent (nuclear-family A2A, D3).
   ...
   - I2.6 **Append-only session logs.** Nodes' logs are immutable history; steering writes new turns, never edits old ones (Prime Agent append-only JSONL sessions — `docs/research/prime-agent.md` §2; opencode durable session history — `docs/research/opencode.md` §1).
...
- The tree is also the **durable conversation structure** the persistent-session model (D3) needs: a node is the unit of session identity, checkpointing, steering, and result aggregation. Prime Agent's session tree (`id`/`parentId`), compaction summaries carrying "goal/constraints/progress/blocked/decisions" forward (`docs/research/prime-agent.md` §2.5), and the named-child list (§4.5) are the proven precedents this delta adopts.
- Without `parent_task_id` the event log cannot answer "who asked for this, and what did it produce" — the core feedback question of the orchestrating LLM. The column is a one-line schema change with outsized audit/debug value.
...
**Status:** **adopt**
**Amends:** `docs/architecture.md` §5.1 (channel invariants), §5.2 (message schema — add `steer`, `admission`, child→parent result messages), §6.4 (checkpoint — session reload), §7.2 (spawn returns admission), §7.1 (lifecycle), §14 (cold-start paragraph — pool deferral re-scoped), §16.3 (proto-AGI lifecycle).
...
2. **`spawn` returns admission immediately (async handle).** Protocol additions to the authoritative schema (`architecture.md` §5.2):
   - Supervisor → Worker: `{"type":"steer", "request_id":..., "session_id":..., "context":"<parent's follow-up / steering turn>"}` — parent direction to an existing session (repeatable).
   - Worker → Supervisor: `{"type":"admission", "request_id":..., "session_id":..., "accepted":true, "monotonic_ms":...}` — sent immediately on accepting `init`, before `ready`; this is the async handle. The spawner does not block on process lifetime; results arrive later as messages.
   - Worker → Supervisor: child→parent result messages — the existing `result` envelope (§5.2) is repurposed as a **message that flows up the tree** (D2), not merely a terminal report.
...
- The Factorio class of failure is a real, observed failure mode of self-improving harnesses (directive-provided; the generic mechanism is documented in the threat model as "canaries protect the metric" — `wt-threat` §4 M5). The canary gate + human-approval-for-out-of-scope-edits is the cheapest structural defense, and it reuses the dataset machinery Cambium already mandates.
...
3. **`docs/research/sandbox-options.md` stays as evidence** (branch `wt-sandbox`; not in main). Its core finding is cited: unprivileged bwrap is **blocked on this host by AppArmor** — `cat /proc/sys/kernel/apparmor_restrict_unprivileged_userns` → `1`, and `unshare -Ur true` → `Operation not permitted` (`sandbox-options.md` §1.2). The bwrap-as-root path is also broken for this uid (uid-1001 files invisible/nobody, §1.5). The document's recommendation table (§4) is retained as the record of why sandboxing was evaluated and dropped from v2 scope.
4. **Threat-model R3 is re-rated "accepted — out of scope".** `wt-threat` §5 R3 ("Sandbox gap on macOS and dev/CI (`NoopSandbox`)", formerly `needs v2`) becomes **accepted — out of scope**: with the sandbox removed from v2 entirely, the cross-platform parity gap (Linux-only effective bwrap, best-effort macOS, noop dev/CI) is no longer a platform inconsistency — the harness simply does not sandbox, and the accepted residual risk is the absence of a kernel-namespace boundary for `run_shell`. The remaining controls are worktree isolation, allowlists, and approval gates.
5. **Threat-model R4 is fixed, not re-rated** — least-privilege worker env. `architecture.md` §7.2 currently spawns workers with `env={**os.environ, ...}` (the entire host environment), contradicting §12.1's stated per-worker allowlist ("the sandbox injects only the env keys the worker is authorized to receive via `--setenv`"). With no sandbox to scrub at the `--setenv` boundary, the spawn path must enforce the allowlist itself:
   ...
   - This resolves `wt-threat` §3.7/R4: a compromised worker can no longer `print(os.environ)` for unrelated secrets, and the egress chain (key → tool output → model context → committed file → merged to `main`) is cut at the source.
...
- The user directive (2026-08-09) removes sandboxing from the harness. The evidence on this host supports the decision as a *deferral, not a regression*: every namespace-based sandbox option is blocked unprivileged by AppArmor (`sandbox-options.md` §1.2), the root-mode workaround is broken for uid-1001 state (§1.5), and the alternatives (Landlock via ctypes, gVisor, nsjail, firejail) are rejected/absent (§1.7–1.8, §3). Retaining `NoopSandbox` in v2 would silently ship "no sandbox" anyway; removing the module makes the posture honest and the containment story unitary.
- The v2 design already carried the claim that sandbox ≠ primary isolation: `architecture.md` §19 lists worktree recovery, fencing, and process-group kill as the liveness/split-brain controls, and the threat model's own summary ranks `run_shell` containment "on Linux (bwrap + network off)" as the only sandbox-dependent piece (`wt-threat` §3.3, §5 R3). D7 re-bases that piece on allowlists + gates.
...
- **Q7.2** Approval-gate shape: per-task static allowlist only (proposed minimum) vs an interactive/per-operation host `approve()` callback. The current design has no per-tool approval gate ("workers are autonomous", `wt-threat` §4 M4); D7 lists approval gates as a containment layer, so the mechanism must be specified. (Owner: orchestrator + `Custos`.)
...
- **Event schema (§6.3):** +`parent_task_id` column (D2); +gate-verdict records (D4, per-session). Both are additive; the existing gap-free `seq` invariant is untouched.
- **IPC (§5.2):** +`steer`, +`admission`, child→parent result messages (D3). Additive; `request_id` correlation is preserved.
...
- `docs/research/worker-coldstart.md` is cited from branch `wt-coldstart`; it is **not in `main`** as of this writing. Its numbers were not re-measured here.
- `docs/research/sandbox-options.md` is cited from branch `wt-sandbox`; **not in `main`**. Its AppArmor claims were verified on this host in that research; not re-run here.
- `docs/research/threat-model.md` is cited from branch `wt-threat`; **not in `main`**.
