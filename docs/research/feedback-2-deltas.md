...
| `docs/research/example-datasets-v1.md`, `custos-asyncio-design.md`, `threat-model.md`, `sandbox-options.md` | `main` | `main` (read-only) |
...
2. **`unified_diff` becomes a first-class upward field.** The `wt-slice` vertical slice already ships `diff` in its `result_envelope` (`vertical-slice-report.md`, `scripts/fake_worker.py`); this delta promotes that shape into `§3.4`/`§5.2`. The diff is capped (proposed 256 KiB; overflow truncates with a `diff_truncated: true` flag).
...
- **Q8b.1** Diff cap and truncation flag: 256 KiB proposal ok, or content-addressed dedup across children (a child whose diff is empty sends `diff: null`)? (Owner: `Nuntius` schema owner.)
...
1. **A module's boundary is defined by typed ports (`typing.Protocol`), not concrete imports.** v1 port set: `LLMProvider` (`call(prompt, tier, temperature) -> response`), `EventSink` (emit the module's decision events), `DatasetStore` (load examples). Adapters implement the ports: `DiffundoAdapter(LLMProvider)` wrapping `Diffundo`; the fake-LLM harness in tests implements the same port (which is how `example-spec.md` §9.1 runs `decide()` with no network).
...
**Amends:** `docs/architecture.md` §6.1 (event store — add the conversation store), §16.2 (session layout — `sessions/<node_id>/` becomes SQLite-backed); `docs/research/design-deltas.md` D2 item 2 (node session log — storage engine).
