...
**Status:** DRAFT — 2026-08-09. Produced against `agents.md`, `docs/architecture.md` (v2.0.0),
and `docs/module-template/*` as they exist on branch `wt-arch` (**templates are under arch
review and may change**; re-sync the references below before acting on this checklist).
...
| Architecture, resolution matrix, optimization strategy | `docs/architecture.md` | §4 module catalog, §17 DSPy-per-module (pinned siblings), §18 resolution matrix (every CRITICAL flaw + mechanism) |
| Module template | `docs/module-template/architecture.md` | the 12-section per-module template |
| Dataset schema/versioning/canaries | `docs/module-template/dataset-format.md` | JSONL envelope, splits, versions, canary taxonomy |
| Reference example spec | `docs/module-template/example-spec.md` | the `should_decompose` module spec |
...
> **Template caveat.** The module templates are in active review on `wt-arch` (uncommitted
> diffs exist against the merged `main` scaffold). If the template files change, re-read
> `docs/module-template/architecture.md` and `docs/module-template/dataset-format.md` before
> filling your module's `architecture.md`; this checklist tracks their current shape.
...
2. Read `docs/architecture.md` §18 (the resolution matrix). Every module must not reintroduce
...
3. Read `docs/architecture.md` §17 (DSPy-per-module strategy): the §17.2 sibling-pinning table,
...
4. Read `docs/module-template/architecture.md` (the normative template) and
   `docs/module-template/dataset-format.md` (normative dataset schema).
...
6. Copy `docs/module-template/architecture.md` to `src/cambium/modules/<name>/architecture.md`
...
7. Assign a module code from the `docs/architecture.md` §4 catalog (or "new" if not listed)
...
1. **Inputs** (§3.1): a typed frozen dataclass (or typed parameters to `forward()`). Untyped
...
`docs/module-template/dataset-format.md`.
...
3. **Layout and minimums** — two permitted shapes; declare which in `architecture.md` §7:
   ...
   |---|---|---|---|
   | **v2.1 split (normative target, `dataset-format.md` §4)** | `datasets/train.jsonl`, `datasets/eval.jsonl`, `datasets/canaries.jsonl` + sidecar `datasets/meta.json` | train **≥ 200**, eval **≥ 50** (held-out), canaries **≥ 15** | eval + canaries frozen (`meta.json` `*_frozen_at`) |
   | **v2 single-file interim (reference shape, `example-spec.md` §7.1)** | `datasets/<name>_pairs.jsonl`, canaries inline with `"canary": true` | **≥ 8 records, ≥ 2 canaries** (reference ships 9/2; the scenario test gates on ≥8/≥1) | canaries frozen by test assertion, not sidecar |
...
1. Have the module adversarially reviewed before merge (agents.md §9 item 6; arch §19.5
   "adversarial review gates"). A new module gets a review committed under `docs/reviews/`
   (shape of the three v0.1 reviews: `docs/reviews/review-{distributed-systems,llm-design,implementation}.md`),
...
Only once the module is merged and has a frozen dataset. Follow `docs/architecture.md` §17.4:
...
Per `docs/module-template/dataset-format.md` §5, two orthogonal versions, both recorded in
`datasets/meta.json`:
...
- `meta.json` is regenerated by `scripts/check_dataset.py`, which fails CI on inconsistencies.
...
1. Per-module changelog in `src/cambium/modules/<name>/architecture.md` §12:
   | Version | Date | Change | (semver of the module's `program.py`; the reference starts at 0.1.0/1.0.0).
...
3. If you assigned a **new module code**, add the row to the catalog in `docs/architecture.md`
...
- [ ] Adversarial review committed under `docs/reviews/` (or an existing review updated and
      re-run). *Verify:* `ls docs/reviews/*<name>*`.
...
8. **Interface drift without catalog/review update.** Changing a module's contract (input
   shape, output shape, error types) without updating `architecture.md` §3, the `docs/architecture.md`
