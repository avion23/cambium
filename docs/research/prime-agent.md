...
2. **Socket + lock-file supervision has real failure modes.** The daemon uses Unix sockets and lock directories (`/tmp/prime-agent-1001/daemon.sock`, `daemon.sock.lock`, `session-leases/*.lock`). Local `agent.jsonl`: **26** `Could not adopt worker` failures (19 `Timed out waiting for daemon worker hello`, 5 `Session worker process is no longer running`, 2 other — 1 `Active session ... is closing`, 1 `connect ECONNREFUSED`), **57** `Supervisor command attach failed: Error: Unknown active session`, and **36** lines containing `Timed out` (of which 20 are `Timed out connecting to daemon session worker`; the remaining 16 are attach/eviction/peer-sync/heartbeat timeouts). This validates Cambium's design doc anti-pattern list ("Unix socket + lock-file supervision", "Prime Agent").
...
6. **Heavy daemon/log sprawl.** 59 `worker-*.log` socket logs under `~/.prime/agent/logs/` (`ls worker-*.log | wc -l` → 59; 62 files total in the dir), plus session-leases lock files for every session; frequent daemon restarts logged (110 `shutting down (exit 0)` lines).
...
4. **Keep stdin/stdout IPC; avoid the socket+lockfile layer.** Cambium's pipe-based Kahn network avoids the attach/hello timeout failures seen here (26 "Could not adopt worker" + 36 "Timed out" + 57 "Unknown active session" in logs).
...
| Daemon timeout/attach failures | `grep -c "Could not adopt worker" …` (→26: 19 hello-timeout, 5 worker-dead, 2 other); `grep -c "Timed out" …` (→36: 20 "Timed out connecting to daemon session worker"); `grep -c "Unknown active session" …` (→57) | `26` / `36` / `57` |
...
3. Local `agent.jsonl`: **886 warn + 6 error** lines; **6** V8 heap-OOM traces; **26** "Could not adopt worker" failures (19 hello-timeout); **36** lines containing "Timed out"; **57** "Unknown active session" attach failures.
