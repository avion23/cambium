...
**Trust boundaries.** The design draws one hard boundary: **worker ↔ host**, enforced by (a) subprocess + stdin/stdout JSON-Lines IPC (`architecture.md` §5.1), (b) the Septum sandbox (Linux bwrap / macOS sandbox-exec / noop — `architecture.md` §4, §19), (c) per-worker git worktree isolation (`architecture.md` §7.5), and (d) generation fencing tokens (`architecture.md` §7.3). Everything outside that boundary — the host process, the environment, the session dir, `main`, and the network — is higher trust.
...
- **Status:** accepted on Linux (bwrap + network off) — see §4 M1. The mitigation **depends entirely on the sandbox**: with the macOS `SandboxExecSandbox` (best-effort) or the dev/CI `NoopSandbox`, there is no effective boundary (see M1, R3).
...
- **What the arch docs say:** not addressed. `architecture.md` §7.5 (worktree recovery) and §11 (tool set) do not mention symlink handling. The only concrete mount set in the corpus is v0.1 M8 (`system-design.md` §M8): bwrap binds the worktree path, ro-binds `/usr /lib /lib64 /bin`, mounts `--dev /dev`, `--proc /proc`, `--tmpfs /tmp`, and `--unshare-net` when network is off.
- **Analysis (Linux):** under bwrap, the root filesystem is empty except the bound paths, so a symlink to a host path not in the bind set resolves to *nothing* (no file, read fails), and writes through symlinks into ro-bound system dirs fail. A symlink to another location *inside* the worktree is harmless. So bwrap largely neutralizes this attack — **provided the mount set is the true namespace**.
...
- **Status:** accepted on Linux bwrap (as analyzed), **needs v2** for the mount-set specification and for macOS/dev (`NoopSandbox`) where the worktree shares the host filesystem. Verification: Test 6.4.
...
|---|---|---|---|---|
| M1 | **Septum sandbox (bubblewrap)** | bwrap binds only the worktree + ro system dirs, `--tmpfs /tmp`, `--unshare-net` when network off (`system-design.md` §M8); v2 adds `BwrapSandbox` / `SandboxExecSandbox` / `NoopSandbox` backends (`architecture.md` §4, §18.3 IMPL-M4); spawn goes through `sandbox.wrap([...])` (`architecture.md` §7.2). | Worker RCE blast radius; network egress (off by default, §11); filesystem read/write scope. | **Linux-only effective.** macOS `SandboxExecSandbox` is "best-effort" and dev/CI is `NoopSandbox` — both documented as weaker (§19). Mount set not normatively specified in v2 (§4) — see §3.5. No seccomp specified; bwrap alone is a kernel-namespace boundary, not a hardened jail. |
...
| **R2** | **`write_file`/`edit_file` path confinement unspecified** | T2/T3 → §3.4 | **High** (arbitrary file write outside worktree under `NoopSandbox`; `${session_dir}` corruption) | **needs v2** | `read_file` rejects out-of-worktree paths (§11) but `write_file`/`edit_file` do not (UNVERIFIED). Under bwrap the blast radius is capped; under dev/CI `NoopSandbox` it is not. |
| **R3** | **Sandbox gap on macOS and dev/CI (`NoopSandbox`)** | T3 → §3.3 | **High on those platforms** (arbitrary shell with no filesystem/network boundary); **Medium on Linux** | **needs v2** (documented limitation) | `architecture.md` §19: "macOS sandbox is weaker than Linux bwrap (documented as best-effort)"; §18.3 IMPL-M4: `NoopSandbox` for dev/CI. On those backends the *only* remaining protection against `run_shell`'s intentional `shell=True` (§11) is the worktree isolation — which is filesystem-namespace-free. Kernel-level bwrap escapes (no seccomp specified) are **accepted** as out of scope. |
| **R4** | **Full host environment inherited by every worker** | T3 → §3.3/§3.7 | **High** (all A1 keys readable in-process; exfiltration chain to committed files) | **needs v2** | `architecture.md` §7.2 spawns workers with `env={**os.environ, ...}` — every host env var, including all API keys, is readable via `os.environ` in any worker, contradicting §12.1's "sandbox injects only the env keys the worker is authorized to receive via `--setenv`." bwrap's `--setenv` only *sets* vars; it does not scrub inherited ones, and no scrubbing mechanism is specified. With network off, egress is blocked, but output can still be written to worktree files and merged (A2/A5). |
...
| **R10** | **`/proc` exposure inside bwrap** | T3 → sandbox | **Low** | **accepted** | v0.1 M8 binds `--proc /proc` without `--unshare-pid`, so a worker can enumerate host processes (and, subject to kernel `ptrace_scope`, inspect same-user peers). Metadata leak only; not in the design's stated protections. |
...
| 4 | **Symlink worktree escape** — create a symlink in the worktree → host path; worker writes through it. Also verify `git_op` works inside the sandbox (gitdir reachable). | M1 + §3.5 mount set | Linux bwrap: writes outside namespace fail; **git_op works** (requires gitdir bound — currently unspecified). Noop/macOS: write lands outside worktree → documents R2/R3. | **Partial / UNVERIFIED** (mount set not specified in v2 §4). |
