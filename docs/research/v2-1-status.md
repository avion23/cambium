...
| Providers | `Diffundo` performs tiered priority ordering with cooldown, circuit-breaker, configured RPM request-rate buckets, and bounded retry behavior. A depleted bucket reports `RATE_LIMITED`; HTTP 429 `Retry-After` is honored. Redacted usage events cover token/cost fields, account-quota ownership, prompt-prefix bytes, and provider cache-hit metrics. `scripts/external-provider-smoke.sh` passed end-to-end against a live codex OAuth provider (ChatGPT `pro`): provider-backed worker loop, real usage events, one ref-only commit, and an unchanged main on the deterministic fail-closed failure fixture. The codex transport sends the CLI identity headers (`originator: codex_cli_rs`, codex-shaped `User-Agent`, stable per-session `session-id`); a live `GET /backend-api/codex/models?client_version=<codex-cli-ver>` returned 200 and listed `gpt-5.6-luna` for the pro session, confirming server-side entitlement to the pinned slug. A `config_error:`/`auth_error:` outcome is now a durable quarantine record in the routing ledger (`routing-state.json`): `DebtStore` stores `disable_reason`/`disable_at` on that failure class, clears both on the next success, and `doctor`'s provider check appends `(disabled: <reason>)` to the row (advisory only — a missing or corrupt ledger never fails the check). `scripts/external-provider-smoke.sh` also passed end-to-end against a live API-key provider (z.ai, `glm-5.3`): it now derives the plan's `provider_env_keys` from each configured provider's `api_key_env`, so the supervisor's fail-closed worker allowlist forwards API-key credentials (values are redacted); codex-chatgpt providers still need no forwarding. Cross-session evidence (`scripts/usage_evidence.py`, 2 sessions × 14 calls per provider): codex and zai both 14/14 success with zero retries and a byte-stable prompt prefix per provider (5243/5236); zai measured ~2.2× lower mean latency (6.84s vs 15.14s) and ~2.6× higher provider-reported cache-hit rate (0.929 vs 0.357) on the smoke workload; neither transport reports cost, so cost-weighted routing has no data yet. A live z.ai probe found the coding-plan endpoint normalizes the requested `glm-5.2` slug to `glm-5.3` in its response — the worker's strict `result.model == model` trust check fails unless the config uses the model name the endpoint reports. | Weighted routing now has stable cross-session, cross-provider usage evidence (latency, provider cache-hit, token efficiency per provider) but no cost data; the routing weights still await a design that uses those measured signals. |
...
| IPC framing | Worker stdout is NDJSON; malformed lines that fail JSON parsing and valid JSON lines that are not objects are counted as `parse_error` and skipped up to the same bound, never failing supervision. | None. |
...
failure reason; un-reported fields omitted); the usage-debt ledger
    ...
    The evidence is now stable across two providers and two sessions each
    (codex `gpt-5.6-luna` and z.ai `glm-5.3`, 14/14 success each); weighted
    routing is the follow-on design on those measured signals.
4. Opt-in external-provider smoke: `scripts/external-provider-smoke.sh` is
    ...
    exist. It drives both codex-chatgpt OAuth providers and API-key providers
    (forwarding each configured `api_key_env` through the worker's fail-closed
    allowlist).
