...
| Providers | `Diffundo` performs tiered priority ordering with cooldown, circuit-breaker, token-bucket quota, and bounded retry behavior. HTTP 429 `Retry-After` is honored. | Durable provider usage, cost, quota ownership, and privacy contract must precede weighted routing. External-provider acceptance is not evidenced. |
...
| Controls | `tools.py` has schemas, command policy, `ApprovalGate`, and `resources.CompileGate`; provider environments are allowlisted and redaction is available. | No production approval callback or per-worker OS containment. `ResourceBudget`, `worker_pool.py`, `dlq.py`, `events.py`, and `eval_cache.py` are not tracked modules. |
...
| Smoke evidence | Root-confirmed loopback smoke used a bounded host scope: two provider requests, result `done`/exit 0, one commit to `smoke.txt`, and events including `write_file`, gate 0, `merge_committed`, and `session_ended`. | No real external-provider run occurred because credentials/configuration are absent. The scope smoke does not prove per-worker OS containment. |
