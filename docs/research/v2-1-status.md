...
| --- | --- | --- |
| CLI and diagnostics | The `cambium` CLI runs directly from source (`PYTHONPATH=src python3.14 -m cambium.cli`); no wheel or install is supported. Commands: `auth`, `supervisor`, `doctor`, `bench`, `tasktree`, `module-test`, `version`, and `inspect` (`inspect --session-dir DIR` reads `.cambium/result.json`). `doctor` checks runtime, worktrees, provider/auth state, optional stores, datasets, and host health. | Diagnostics are not production admission or approval. |
