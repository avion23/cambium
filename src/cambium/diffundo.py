...
def _provider_cache_hit(usage: dict[str, Any] | None) -> bool | None:
    ...
    True when the normalized cached-token count is positive; False when the
    provider reports a zero count; None when cache evidence is absent. This
...
class Diffundo:
    ...
    def _summary_router(self) -> Diffundo:
        ...
        # A semantic summary is a separate branch. It can use any eligible
        # provider and must never inherit coding ownership or its selection
        # state. The summary's local success/failure state is discarded
        # after the call, while shared admission evidence remains merged at
        # the accounting boundary below.
        ...
        # Summary exhaustion is local to this semantic branch. Do not let
        # summary-only failures trip or reset the coding branch's circuit.
        summary._all_provider_failure_lock = threading.Lock()
        summary._consecutive_all_provider_failures = 0
