...
class ExampleDatasetLoader(DatasetLoader):
    ...
    {"review": bool, "decompose": bool, "reason"}}``. The loader maps the
    wire boolean ``expected.review`` to :class:`Decision` in
    ``Example.expected``. ``expected.decompose`` is the generic v1
    class-balance mirror consumed by the bench harness; it must agree with
    ``review``. The optional boolean ``canary`` field flags dataset-integrity
    entries planted to catch reward hacking in future evals; they are loaded
    and scored like any other entry by :meth:`load`. Split-aware loads
    ...
    def load_split(self, split: Split) -> list[Example]:
        ...
        Falls back to ``should_review_pairs.jsonl`` (the v2 flat format, no
    ...
    def _load_split(self, split: Split, meta: dict) -> list[Example]:
        ...
        if split_path.is_file():
            ...
            fallback = self.datasets_dir / "should_review_pairs.jsonl"
    ...
    def _validate(self, record: dict, line_no: int, path: Path) -> None:
        ...
        if not isinstance(decompose, bool) or decompose != record["expected"]["review"]:
            raise DatasetError(
                f"{path}:{line_no}: expected.decompose must mirror expected.review "
