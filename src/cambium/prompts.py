...
CODING_POLICY = (
    ...
    "as evidence instead of repeating calls. Keep trivial lookups and one-turn read-only probes "
    "local. Batch known independent reads in one action instead of creating children for them. "
    "Delegate substantial independent reasoning, editing, or long verification together in one "
    "parallel batch, with one bounded child per target. Set scope-appropriate max_turns and "
    "max_wall_s on each delegate; explicit values win and omitted parallel budgets use "
    "max_turns=8/max_wall_s=300 for read-only investigations or "
...
_ACTION_PROTOCOL = (
    ...
    "Independent reads may run concurrently; mutations run in listed order. Keep trivial lookups "
    "and one-turn read-only probes local. Batch known independent reads in one action (prefer one "
    "read_batch call for several paths) instead of delegating them. Delegate substantial "
    "independent reasoning, editing, or long verification together in one parallel delegate batch, "
    "with one bounded child per target; do not serialize those branches. Each delegate spec needs "
    "a self-contained task and scope-appropriate max_turns and max_wall_s. For model-facing "
    "parallel delegate batches (siblings > 1), omitted budgets default to "
    "max_turns=8/max_wall_s=300 for read-only investigation children and "
    "max_turns=12/max_wall_s=600 for writable children; "
    "explicit values always win. These batch defaults do not apply to a single child. One child "
    "defaults to trunk+inherit; several writable children in a batch default to semantic+spread; "
    "read-only investigation siblings use fresh+spread (one probe uses fresh+inherit). Optional "
