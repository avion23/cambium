        ...
        for turn in range(first_turn, config.max_turns + 3):
            ...
            if isinstance(fallback_origin, str) and result.provider != last_provider:
                ...
                correction = {
                    "role": "user",
                    "content": _bounded_text(
                        f"Note: the assigned provider was unavailable; the provider "
                        f"serving this turn is {result.provider}/{result.model}.",
                        MAX_OBSERVATION_BYTES,
                    ),
                }
                if base_messages is None:
            ...
            try:
                ...
                invalid_messages = [
                    {"role": "assistant", "content": assistant_content},
                    {
                        "role": "user",
                        "content": _bounded_text(
                            f"invalid action: {exc}. For one tool use "
                            '{"name":"read_batch","arguments":{"paths":["file.py"]}} '
                            "with the intended name and arguments; omit batch nesting. "
                            'For completion use {"type":"finish","summary":"...",'
                            '"objective_met":true} (false when incomplete). No prose.',
                            MAX_OBSERVATION_BYTES,
                        ),
                    },
                ]
                ...
                if writer is not None:
                    ...
                    await send(
                        writer,
                        {
                            ...
                            "generation": config.generation,
                            "turn": turn,
                            "error": _cap_utf8(str(exc), MAX_ENVELOPE_FIELD_CHARS),
