...
def test_default_worker_spawns_exact_module_argv(tmp_path) -> None:
    session_dir = tmp_path / "session"
    repo = session_dir / "repo"
    base = _make_repo(repo, {"a.txt": "file a\n"})
    plan = {
        "tasks": [
            _task(
                session_dir, repo, base, "t-default", worktree="wt-default",
                branch="wt-default", target_file="a.txt", marker="// cambium-default",
                gate="grep -q '// cambium-default' a.txt",
            )
        ]
    }
    ...
    spawned = _spawned(events)
    assert len(spawned) == 1
    assert spawned[0]["payload"]["worker"] == " ".join(
        [sys.executable, "-u", "-m", "cambium.worker"]
    )


def test_default_worker_is_the_module_literal(tmp_path) -> None:
    session_dir = tmp_path / "session"
    repo = session_dir / "repo"
    base = _make_repo(repo, {"a.txt": "file a\n"})
    plan = {
        "tasks": [
            _task(
                session_dir, repo, base, "t-literal", worktree="wt-literal",
                branch="wt-literal", target_file="a.txt", marker="// cambium-literal",
                gate="grep -q '// cambium-literal' a.txt",
